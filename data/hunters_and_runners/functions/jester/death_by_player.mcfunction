...
execute if score jesters_on_death hnr.settings matches 0 if entity @a[scores={hnr.killed=1..},team=runners] as @a[scores={hnr.killed_by=1..}] run team join runners @s
#> If the killer is a hunter, as the killed join the hunters team
execute if score jesters_on_death hnr.settings matches 0 if entity @a[scores={hnr.killed=1..},team=hunters] as @a[scores={hnr.killed_by=1..}] run team join hunters @s

#> If jesters_on_death == 1, Let Jester decide their role
#execute if score jesters_on_death hnr.settings matches 1 tellraw @s 
