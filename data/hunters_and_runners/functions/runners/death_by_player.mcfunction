...
##> If the killer is a jester, as the killer tell the killed who they were killed by, then make Jester take the runners role
...
execute if entity @a[scores={hnr.killed=1..},team=jester] as @a[scores={hnr.killed_by=1..}] run function hunters_and_runners:runners/join_team
execute if entity @a[scores={hnr.killed=1..},team=jester] as @a[scores={hnr.killed_by=1..}] run title @s subtitle {"text":"You are now a runner","color":"aqua"}
